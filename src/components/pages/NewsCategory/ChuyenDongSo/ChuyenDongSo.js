...
import style from './ChuyenDongSo.module.scss'
...
function ChuyenDongSo() {
    const [title, setTitle] = useState('Chuyển động số')
    const [type, setType] = useState('chuyen-dong-so-1218')
        ...
        <div className={style['wrapper']}>
            ...
            <div className={style['content']}>
                <div className={style['breadcrumb']}>
                    <ol>
                        <li>Chuyển động số</li>
                        {title==='Chuyển động số'?'':(<li className={style['breadcrumb-active']}><span>/</span> {title==='Chuyển động số'?'':title}</li>)}
